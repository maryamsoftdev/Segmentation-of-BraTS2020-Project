...
flair_path = TRAIN_DATASET_PATH + 'BraTS20_Training_355/BraTS20_Training_355_flair.nii'
try:
    test_image_flair = nib.load(flair_path).get_fdata()
    print("FLAIR image loaded successfully.")
    print(f"Max value in FLAIR image: {test_image_flair.max()}")
    ...
    print(f"An error occurred while loading FLAIR image: {e}")

# Load other images similarly with error handling
def load_and_scale_image(image_path):
    try:
        image = nib.load(image_path).get_fdata()
        image = scaler.fit_transform(image.reshape(-1, image.shape[-1])).reshape(image.shape)
        print(f"Image loaded and scaled: {image_path}")
        return image
    except FileNotFoundError:
...
test_image_t1 = load_and_scale_image(TRAIN_DATASET_PATH + 'BraTS20_Training_355/BraTS20_Training_355_t1.nii')
test_image_t1ce = load_and_scale_image(TRAIN_DATASET_PATH + 'BraTS20_Training_355/BraTS20_Training_355_t1ce.nii')
test_image_t2 = load_and_scale_image(TRAIN_DATASET_PATH + 'BraTS20_Training_355/BraTS20_Training_355_t2.nii')

# Load mask with error handling
mask_path = TRAIN_DATASET_PATH + 'BraTS20_Training_355/BraTS20_Training_355_seg.nii'
try:
    test_mask = nib.load(mask_path).get_fdata().astype(np.uint8)
    print(f"Mask image loaded successfully: {mask_path}")
    ...
    print(f"An error occurred while loading mask: {e}")
import random
...
print(combined_x.shape)#Do the same for mask
